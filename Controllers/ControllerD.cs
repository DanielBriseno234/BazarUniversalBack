    ...
    public class ControllerD : Controller
    {
        ...
        public async Task<IActionResult> GetItems(string q)
        {
            var query = await (from p in _baseDatos.Products
                               join b in _baseDatos.Brands on p.BrandId equals b.Id.ToString()  // Realizamos el join con la tabla de marcas
                               join c in _baseDatos.Categories on p.CategoryId equals c.Id.ToString()  // Realizamos el join con la tabla de categorías
                               where p.Title.Contains(q)  // Filtro por título o descripción
                               select new
                               {
                                   p.Id,
                                   p.Title,
